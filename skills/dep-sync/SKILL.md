...
```bash
dep dap resolve "sync documentation" --json --root <project-root>
```
...
```bash
dep dap node sync-stale-docs check-staleness --json --root <project-root>
```
...
```bash
dep dap node sync-stale-docs <next-node-id> --json --root <project-root>
```
