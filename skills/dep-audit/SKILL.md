...
```bash
dep dap resolve "audit documentation" --json --root <project-root>
```
...
```bash
dep dap node audit-existing-docs inventory-docs --json --root <project-root>
```
...
```bash
dep dap node audit-existing-docs <next-node-id> --json --root <project-root>
```
